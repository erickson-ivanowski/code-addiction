...
IF BRANCH_TYPE = feature AND REVIEW_MD NOT FOUND AND NOT --force:
  ...
  ✅ DO: Inform user to run /add.check first

IF BRANCH_TYPE = feature AND REVIEW_MD OVERALL = BLOCKED AND NOT --force:
  ⛔ DO NOT USE: Write to create changelog.md
  ⛔ DO NOT USE: Bash for done.sh --merge
  ✅ DO: Show blocked gates and request /add.check re-run
...
2. IF NOT EXISTS: "Review not executed. Run /add.check before /add.ship." -> BLOCKED
...
2. Force merge anyway: /add.ship --force
```
...
3. Force merge anyway: /add.ship --force
```
...
After PR created, inform: "PR Created! Merge via GitHub when approved. After merge, run: /add.ship (will detect merged PR and cleanup)"
