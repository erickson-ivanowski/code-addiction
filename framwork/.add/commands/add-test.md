...
```json
{
  ...
  "patterns": {"if_exists": ".add/project/*.md", "action": "READ for project context"}
}
```
...
**IF MODE = feature:**
1. RUN: `bash .add/scripts/feature-status.sh` (if exists)
...
1. READ .add/skills/backend-development/SKILL.md (if exists) for project patterns
2. READ .add/project/*.md (if exists) for project conventions
...
## MANDATORY: Load Context
1. READ .add/skills/frontend-development/SKILL.md (if exists) for project patterns
2. READ .add/project/*.md (if exists) for project conventions
...
## MANDATORY: Load Context
1. READ .add/skills/backend-development/SKILL.md (if exists)
2. READ .add/project/*.md (if exists)
...
IF `.add/scripts/log-iteration.sh` exists:
```bash
bash .add/scripts/log-iteration.sh "test" "add-test" "Generated tests — [CURRENT_COVERAGE]% coverage" "[ALL_TEST_FILES comma-separated]"
```
